...
data_cols= ['period_init', 'v_kick', 'bound?', 'tighten?', 'circularize?']
...
def trial(module='circular', period=100, v_kick=50, e=0, m1=10, m2=10):
    ...
    return np.array([T_init, v_kick, bound, tighten, circularize])
...
with ProcessPoolExecutor(max_workers=6) as executor:
    ...
    for i, (period, v_kick) in enumerate(tqdm(config_list, desc="Configs")):
        ...
        bound_values = [r[2] for r in chunk_results]
        tighten_values = [r[3] for r in chunk_results if (r[2] == 1)]
        circularize_values = [r[4] for r in chunk_results if (r[2] == 1)]

        bound_prob = np.mean(bound_values)
        tighten_prob = np.mean(tighten_values) if tighten_values else np.nan
        circularize_prob= np.mean(circularize_values) if circularize_values else np.nan
        
        summary_results.append([period, v_kick, bound_prob, tighten_prob, circularize_prob])
    
modelData = pd.DataFrame(summary_results, columns=data_cols)
modelData.to_csv(f"data_resol/{MODULE}_v2.csv", index=False)